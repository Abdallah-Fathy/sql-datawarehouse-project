...
Parameters: 
  None.
  This Stored Prcedures does not accept any parameters or return values.
...
BEGIN
	BEGIN TRY
		...
		SELECT 
			...
			CASE
				...
				ELSE 'n/a'  -- Normalize matiral status values to readable format
			END cst_gndr,
			cst_create_date
		FROM (
		...
		SELECT 
			...
			CAST (
				  ...
				  AS DATE
			) AS prd_end_dt -- Calculate end date as ine day before the next start date 
	...
	BEGIN CATCH
		PRINT '====================================================';
		PRINT 'ERROR ORCURED DURING LOADING SILVER LAYER';
